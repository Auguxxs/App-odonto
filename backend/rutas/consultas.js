...
async function listarPacientes() {
  const entidad = "pacientes";
  try {
    const respuesta = await fetch(`${url}/${entidad}`);
    const pacientesDelServidor = await respuesta.json();
    if (Array.isArray(pacientesDelServidor)) {
      pacientes = pacientesDelServidor;
    }
    if (respuesta.ok) {
      pacientes.forEach((_mascota, indice) => {
